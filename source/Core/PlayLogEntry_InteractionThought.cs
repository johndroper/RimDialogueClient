...
#pragma warning restore CS8618
    ...
    protected override string ToGameStringFromPOV_Worker(Thing pov, bool forceLog)
    {
      ...
      if (Settings.VerboseLogging.Value)
        Mod.Log($"Entry {LogID} - Generating InteractionThought {InteractionDef?.GetType().Name} {initiator} -> {pov}");
      ...
      if (Settings.VerboseLogging.Value)
        Mod.Log($"Entry {LogID} - Creating InteractionThought dialogue request.");

      if (InteractionDef == null)
        throw new System.Exception("PlayLogEntry_InteractionThought has a null InteractionDef reference.");
